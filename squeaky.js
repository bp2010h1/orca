...
Class = function(attrs) {
	...
	newClass.prototype._block = function(func) {
		var retVal = func();
	...
	// wrapper for blocks
	newClass.prototype.$objectPrototype.prototype._block = function(func) {
		var retVal = func();
	...
	if('superClass' in attrs) {
		...
		newClass.prototype.$objectPrototype.prototype = attrs['superClass']._new();
		newClass.prototype.$objectPrototype.prototype._superClass = attrs['superClass'];
		...
		newClass.prototype.$objectPrototype.prototype._super = function(method, args) {
			// super calls methods without invoker for avoiding infinite recursion because
			// just the invoker comes from the superclass, the invoked method comes from the current class
			return this._superClass.$objectPrototype.prototype['_' + method].apply(this, args);
		};
	}
	...
	if('instanceVariables' in attrs) {
		...
		for(idx in attrs['instanceVariables']) {
			newClass.prototype.$objectPrototype.prototype[attrs['instanceVariables'][idx]] = null;		
		}
	}
	...
	if('instanceMethods' in attrs) {
		...
		for(method in attrs['instanceMethods']) {
			newClass.prototype.$objectPrototype._createMethod(method, attrs['instanceMethods'][method]);
		}
	}
